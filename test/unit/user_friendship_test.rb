...
class UserFriendshipTest < ActiveSupport::TestCase
	...
	test "that creating a friendship works" do 
		assert_nothing_raised do 
			UserFriendship.create user: users(:arkadiusz), friend: users(:mike)
		end	
	end

	test "that creating a friendship based on user id and friend id works" do 
		UserFriendship.create user_id: users(:arkadiusz).id, friend_id: users(:mike).id
		assert users(:arkadiusz).pending_friends.include?(users(:mike))
	end

	context "a new instance" do 
		setup do 
			@user_friendship = UserFriendship.new user: users(:arkadiusz), friend: users(:mike)
		end
	...
	context "#send_request_email" do 
		setup do 
			@user_friendship = UserFriendship.create user: users(:arkadiusz), friend: users(:mike)
		end
	...
	context "#mutual_friendship" do
		setup do
			UserFriendship.request users(:arkadiusz), users(:mike)
			@friendship1 = users(:arkadiusz).user_friendships.where(friend_id: users(:mike).id).first
			@friendship2 = users(:mike).user_friendships.where(friend_id: users(:arkadiusz).id).first
		end
	...
	context "#accept!" do
		setup do 
			@user_friendship = UserFriendship.request  users(:arkadiusz),  users(:mike)
		end
	...
	context "#block!" do
		...
		should "not allow new request once blocked" do
			@user_friendship.block!
			uf = UserFriendship.request users(:arkadiusz), users(:mike)
			assert !uf.save
		end
	end
